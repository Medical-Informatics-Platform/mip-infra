...
type: Opaque
stringData:
  platform-backend-db.DB_ADMIN_USER: CHANGEME_TO_A_UNIQUE_ONE_PER_FEDERATION
  platform-backend-db.DB_ADMIN_PASSWORD: CHANGEME_TO_A_UNIQUE_ONE_PER_FEDERATION
  platform-backend-db.PLATFORM_DB_USER: CHANGEME_TO_A_UNIQUE_ONE_PER_FEDERATION
  platform-backend-db.PLATFORM_DB_PASSWORD: CHANGEME_TO_A_UNIQUE_ONE_PER_FEDERATION
```
...
- Creates a unique `mip-secret` **per** federation with autogenerated 24-character alphanumeric values for all required keys.
