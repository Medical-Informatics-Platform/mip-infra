...
type: Opaque
stringData:
  platform-backend-db.DB_ADMIN_USER: ${PB_ADMIN_USER}
  platform-backend-db.DB_ADMIN_PASSWORD: ${PB_ADMIN_PASS}
  platform-backend-db.PLATFORM_DB_USER: ${PB_USER}
  platform-backend-db.PLATFORM_DB_PASSWORD: ${PB_PASS}
EOF
    if apply_yaml_file "$ns" "$f"; then
